...
# To compute the new total of workers we multiply the original weigths by the growth rate. 
new_total_n <- format(sum(df$hhwgt[df$pop_of_int==1] * 
                            (1 + workers.gr)^4, 
                          na.rm = TRUE), big.mark=",")
# Here we adjust min wages
# SAME
...
frac_to_destroy <- df %>% 
  ...
  summarise(2 * delta.e1 * 1e6 / sum(hhwgt.2016))


add.nocomp <- function(df.temp=df, alpha.1.temp = frac_to_destroy) {
  #randomly assign no compliance to alpha.1.temp% of the population
  df.temp %>% mutate("wage_cut" = ifelse(pop_of_int==1 & wages.final<10.10,   
                                    ...
                                    NA), 
  #Assing old wages for all the non-compliers
                "new.wage.nocomp.halfwage" = ifelse(wage_cut %in% "half wage" , 
                                    wages.final/2, 
                                    new.wage.nocomp) 
                                    ) 
}
...
df %>% filter(pop_of_int == 1 & wages.final <10.10) %>% summarise(0.03*sum(!is.na(hhwgt.2016))) 
