...
#################
st.minw.2016 <- state_min_w %>% select( c( "state_co", "y2016", "state") )%>% rename(state_la = state, state = state_co)

df.temp1 <- df.temp %>% mutate(state_la = as.character(as_factor(state)), state = as.numeric(state))

asd <- left_join(df.temp1,st.minw.2016, "state")
table(asd$y2016, useNA = "always")
#########
