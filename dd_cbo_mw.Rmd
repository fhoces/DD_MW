...
The formula for the adjustments are the same as those propose in equation XXXX
...
<a id="displayText" href="javascript:toggle(34);">`R`</a>  
...
inc_ch_stats <- function() {
  res1 <- list()
  res1[["losses"]] <- df %>% 
    summarise(sum(winners * hhwgt.2016) - 
              param.factor.1 * sum(losers * hhwgt.2016) -
              param.net.benef)  
  aux1 <- df %>% mutate("poverty_bins" = 
                        findInterval(x = sq.inc.pc, vec = c(-Inf,11740, 6*11740, Inf))) %>%
...
output.template2.final_f <- function(){
  cbind(t(output.template2),
        "Replication loses" =  -round((table_9[ "other loses", ]), 1)
...
```{r print final output, eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(
  list(
...
Say something about alternative 1

```{r alt_output1, eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE}
print(final_fig1)
```  

Say something about alternative 2

```{r alt_output2, eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE}
print(final_fig2)
```  

Say something about alternative 3

```{r alt_output3, eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE}
print(final_fig3)
```  
...
### Compute the gap between average wage growth and the growth of the lowest decile (assume by CBO)  
half.gap <- half.gap.f(SA.wage.gr = param.wage.gr, SA.base.growth = param.base.growth)
### 10 wage growth bins starting from lowest assumed value to 2 times the gap computed above
wage.gr.bins <- wage.gr.bins.f(SA.base.growth = param.base.growth, SA.wage.gr = param.wage.gr)
### Apply differential growth rate to wages (by deciles)
df <- wages.final.cps.org.f(SA.states.raise = param.states.raise, SA.wages = param.wages)
