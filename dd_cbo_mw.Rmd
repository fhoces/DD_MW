...
wages.final.cps.org.f <- function(df.temp = df, 
    ...
    # Get state min wages for 2016
    st.minw.2016 <- state_min_w %>% 
      select( c( "state_co", "y2016", "state") )%>% 
    ...
    # merge with state MW
    df.temp <- left_join(df.temp, st.minw.2016, "state")
...
table_1 <- cbind(f_table1(final_weights), 
                 f_table1(!is.na(final_weights)))
colnames(table_1) <- c("N", "Unweighted")
table_2 <- f_table2()
table_3 <- f_table3()
table_4 <- f_table4()
```
