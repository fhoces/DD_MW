...
n_final <- f_n_final()
adj_fact <- f_adj_fact(param.fract.minwage, param.av.wage.var)
elas_final <- f_elast(var_adj_fact = 4.5)
delta.e1 <- f_delta_e()



#######REVIEW AND DELETE
# FUNCTION FOR FINAL EFFECT ON EMPLOYMENT
...
delta.e2 <- sum( (1 + workers.gr)^3 * 
                   ...
                   (1/rep(table.n.final["% Salary below new MW ($P(\\hat{w} \\leq MW^{1})$)","Teen"]/100, 2) * param.fract.minwage) *                      ( 0.39 / ( rep(stats3["$\\overline{\\%\\Delta w}$","Teen"]/100,2)  * param.av.wage.var ) ) *
...
delta.e3 <- sum( (1 + workers.gr)^3 * 
                   ...
                   ( 1 - table.n.final["% of non compliers ($\\alpha_{1}$)",1:2]/100 - 0) * param.noncomp *
                    c( eta.lit * factor.extrap , eta.lit ) *  
                   (1/(table.n.final["% Salary below new MW ($P(\\hat{w} \\leq MW^{1})$)",1:2]/100) * param.fract.minwage) * 
