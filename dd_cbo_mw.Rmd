...
/Users/fhocesde/sample_pref.png
git config --global http.sslCAInfo “/Users/fhocesde/smcacert.crt”
...
list.of.packages <- c("foreign", "weights", "survey", "Hmisc", 
                      ...
                      "reshape2", "tidyverse", "xtable", "haven", "here")
...
compute_rates <- function(var.df) {
  ...
  row.names(var.df) <- c( "wages(total)", "workers", "Price Index, Personal Consumption")
  
  #Generate wage and non-wage income per worker
  var.df <- rbind( var.df ,  
                      (var.df["wages(total)", ] * 1e9 ) / ( var.df["workers", ] * 1e6) )
  
  row.names(var.df) <- c("wages(total)", "workers", "Price Index, 
                          Personal Consumption", "wages per worker")
  # Transpose the data
  var.df <- t(var.df)
...
if ( !("minwage" %in% dir()) ) {
  ...
  aux.1 <- readHTMLTable(xData)
  
  tbls <- html_nodes(fileURL, "table")
...
asd <- html_table(tbls, fill = TRUE)
df1 <- asd[[3]]
df2 <- asd[[4]][,1:5]
df3 <- left_join(df1, df2, "State or other\r\n      jurisdiction")
df3$states <- c("Federal","AL","AK","AZ","AR","CA","CO","CT",
...
df3 <- df3[,-1]
df4 <- df3 %>% select(-"states") 
for (year in as.character(2007:2017)) {
  aux2 <- gsub("[^\\d |^\\. |-]+", "",  df4[, year], perl = TRUE)
  df4[, year] <- unlist(lapply(str_split(aux2, "-"), function(x) max(as.numeric(x))))
}
df4 <- cbind(df3$states, df4)  
...
# Ther state.minw() has a bug where it will not import DC correctly
st.minw.2016 <- state.minw("2015")
st.minw.2016[c("AK"	, "AZ", "CA", "CT"	, "HI"	, "IL", "MA", "MI"	, 
       "MN"	, "MT" , "NV" , "NE" , "NY"	, "OH"	, "RI", "VT", "DC"), ] <- 
          c(  9.75	, 8.05, 10	, 9.6	  , 8.5	  , 8.25, 10	, 8.5	  , 
      9		  , 8.05 , 8.25 , 9		 , 9		, 8.1	  , 9.6 , 9.6, 10.50)
colnames(st.minw.2016) <- "2016"

# Some places report a min wage below the federal one, so we replace
st.minw.2016[st.minw.2016<7.25] <- 7.25
...
Following the original report (footnote 25) we will consider salary workers as non compliers only if their wage is strictly less than 25 cents for non-tipped workers or 13 cents for tipped workers. 
...
stats3 <- final.other.comp(df.temp = df)
...
call.cps.asec.data <-  function() {
  ...
  # Checking if working directory contains data, download if not. 
    ...
    df <- haven::read_dta("cepr_march_2013.dta")
  }
...
Additional information needed from CBO: about this adjustment.  
...
**Information needed from CBO**. So far I am destroying jobs uniformly across workers earning less than 10.10  
...
all.income.f <- function(df.temp = df, non.wage.gr.temp = non.wage.gr) {
  ...
  df.temp$non.wage.2016 <- df.temp$non.wage * non.wage.gr.temp
...
pop.dist <- wtd.table( with(df, findInterval(x = sq.inc.pc, 
                                             vec = c(-Inf,11740, 6*11740, Inf)) ) , weights = df$hhwgt.2016 )$sum.of.weights
...
win.loss.f <- function(SA.factor.1 = param.factor.1, 
  ...
  quintiles <- with(df.temp,wtd.quantile(x = sq.inc.pc, probs = 1:4/5, weights = hhwgt.2016))
  
  #df$income.group <- with(df, findInterval(x = sq.inc.pc, vec =  quintiles ))
  df.temp$income.group.1 <- with(df.temp, findInterval(x = sq.inc.pc, vec =  c(-Inf,11770*c(1,3,6),Inf)  ) )
  return(df.temp)
}
...
SA.part1 <- function(
  ...
  param.jobcut = 1){
    ...
    df <- wages.final.cps.org.f(SA.states.raise = param.states.raise, 
                                ...
                                df.temp = df)
    #table
    stats2 <- N.final.f(df.temp = df, workers.gr.temp = workers.gr)
    #scalar
    eta.lit <- eta.lit.f(SA.eta.lit = param.eta.lit)
    #scalar
  ...
  #scalar
    ...
    return(list("delta.e1" = delta.e1, "stats2" = stats2))
}
