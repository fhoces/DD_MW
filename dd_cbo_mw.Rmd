...
#### Adjusting wages 2 MOVE PAST TABLE 8
...
all.income.f <- function(df.temp = df, non.wage.gr.temp = non.wage.gr) {
  ...
  # Generate per cap income based on new wages (after policy)
  ...
  # Generate pc inc before the new wages  
  df.temp <- df.temp %>% mutate("year.wage.2" = wages.final * (hrslyr * wkslyr) ) %>% 
        group_by(hhseq) %>% mutate("hhld.wage" = sum(year.wage.2, na.rm = TRUE),
                                   ...
                                   "N.fam"= n(), 
                                    ...
                                    select(-(hhld.wage:N.fam) ) %>% ungroup()
...
### Other income losses ADD MORE EMPHASIS   
