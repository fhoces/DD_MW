...
# DESCRIBE (MAYBE WRAP IN FUNCTION)
# Get state min wages for 2016 and 2013
st.minws <- state_min_w %>% 
  select( c( "state_co","y2013" ,"y2016", "state") )%>% 
  rename(state_la = state, state = state_co)
...
Additional information needed from CBO: parameters for this adjustment.  
...
#add pop_of_int as a var to df
df$wage <- as.numeric(df$wage)
df$wages.final <- as.numeric(df$wage)

df <- df %>% filter(!is.na(hhwgt))
# WHY TABLE 4?
...
table_8[2,1] <- round( with(df[(1*pop_of_int == 1) & !is.na(df$wage), ], 
